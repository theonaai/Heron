  ...
  "systems": [
    {
      ...
      "dataSensitivity": "Data classification — ONLY based on agent's statements. If the agent reads names, emails, profile URLs, or job titles, classify as PII even if the agent calls it 'non-sensitive'. Apply the HIGHEST sensitivity across all data the system handles (read AND write). This is the human-readable prose basis for dataSensitivityTier below.",
      "dataSensitivityTier": "T1 | T2 | T3 — the structured tier for this system's data. Taxonomy source: W3C DPV (Data Privacy Vocabulary). T1 = standard / non-personal operational data. T2 = sensitive personal data is present (names, emails, contacts, communication content, employment, customer records) — DPV PersonalData. T3 = a GDPR Article 9 special category is present (health/medical, biometric, genetic, racial/ethnic origin, political opinions, religious/philosophical beliefs, trade-union membership, sex life/sexual orientation) — DPV SpecialCategoryPersonalData — OR financial credentials (payment/bank) OR government IDs (SSN/passport/national ID/tax ID). CRITICAL — classify on what the data ACTUALLY contains: if the agent states a data type is NOT present ('no student names were found', 'no PII'), do NOT raise the tier for it. Apply the HIGHEST tier across all data the system actually handles. This is the value Heron's per-system risk model keys off — emit it for every system.",
