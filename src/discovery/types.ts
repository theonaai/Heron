...
export interface DiscoveryResult {
   ...
   * Renumbered to L3 in docs/comments after AAP-100 dropped the
   * legacy L3 (macOS Keychain) + L4 (cross-cutting OS credentials)
   * readers, but the field name stays `workspaceEnv` for back-compat
   * with persisted `report.json` blobs.
   */
  workspaceEnv?: WorkspaceEnvFile[];
  /** AAP-67 — warnings emitted by readers (e.g. workspace-env parse failures). */
  warnings?: string[];
}

// ─── Workspace .env finding shape (AAP-67) ───────────────────────────────
//
// The reader follows the same names-not-values contract as the L1/L2
// inventory: it returns identifying tokens (variable names) and NEVER
// credential values. Tests assert this invariant by deep-grep across
// the serialized output for known fixture secret patterns.

/**
 * One workspace `.env*` file. `keys[]` is the variable NAMES only.
