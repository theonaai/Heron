...
/**
 * Inline `<link rel="icon">` tag carrying the Heron favicon as a
 * `data:` URI. Previously this pointed at the server route
 * `/favicon.svg` — fine when the document was served by `heron serve`,
 * but broken when a `.html` file produced by `heron scan --format html`
 * was opened directly off disk (e.g. a DPO opening an email
 * attachment). A `data:` URI keeps the doc truly self-contained — no
 * server, no network, no broken icon.
 *
 * The URI is base64-encoded so the SVG contents (which contain `<`
 * and `>` and other URI-unfriendly bytes) survive any downstream HTML
 * processing without further escaping.
 */
export const FAVICON_LINK = `<link rel="icon" type="image/svg+xml" href="data:image/svg+xml;base64,${Buffer.from(HERON_FAVICON_SVG, 'utf-8').toString('base64')}">`;
 ...
 *
 * Self-contained: every asset (CSS, favicon, header logo) is inlined.
 * The favicon is a `data:image/svg+xml;base64,…` URI, so a `.html`
 * file dropped on disk renders correctly without a server and without
 * any network requests — required for DPO email attachments / audit
 * exports.
 */
